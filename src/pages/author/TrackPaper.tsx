...
export default function TrackPaper() {
  ...
  const [selectedRevisionFile, setSelectedRevisionFile] = useState<File | null>(
    null,
  );
  ...
  const fetchPayment = () => {
    fetch(`${url}/payments/paper/${paperId}`, {
      ...
      .then((r) => r.json())
      .then((d) => {
        if (d.success) setPayment(d.payment);
      })
      .catch(() => {});
  };
  ...
  const handleRevisionUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!["docx", "pdf", "tex", "latex"].includes(ext || "")) {
      toast({
        title: "Invalid file",
        description: "Only .docx, .pdf and .tex/.latex allowed.",
        variant: "destructive",
      });
      return;
    }

    setSelectedRevisionFile(file);
  ...
  const confirmRevisionUpload = async () => {
    if (!selectedRevisionFile) return;
    ...
    try {
      ...
      fd.append("manuscript", selectedRevisionFile);
      fd.append(
        "version_number",
        String((data?.latest_version_number || 1) + 1),
      );
      ...
      toast({
        ...
        description: "Your revised manuscript has been uploaded.",
      });
      ...
      setUploading(false);
      setSelectedRevisionFile(null);
    }
  };

  const cancelRevisionUpload = () => {
    setShowRevisionConfirm(false);
    setSelectedRevisionFile(null);
  };
  ...
  const handleReceiptUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!["jpg", "jpeg", "png", "pdf"].includes(ext || "")) {
      toast({
        title: "Invalid file",
        description: "Only JPG, PNG or PDF receipts accepted.",
        variant: "destructive",
      });
  ...
  const confirmReceiptUpload = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${url}/payments/paper/${paperId}/upload-receipt`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
          body: fd,
      ...
      toast({
        ...
        description: "Awaiting publisher review.",
      });
      fetchTracking();
      fetchPayment();
    } catch (err) {
  ...
  const saveMetadata = async (field: "title" | "abstract", value: string) => {
    ...
    try {
      ...
      toast({
        title: "Saved",
        description: `${field === "title" ? "Title" : "Abstract"} updated successfully.`,
      });
      fetchTracking();
  ...
  return (
    ...
    >
        ...
        <div className="p-6 max-w-3xl mx-auto">
          ...
          <div className="mb-6 space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {(paper.issue_label ||
                (paper.issue_volume && paper.issue_number)) && (
                <span className="text-sm text-muted-foreground">
                  ·{" "}
                  {paper.issue_label ||
                    `Vol ${paper.issue_volume}, Issue ${paper.issue_number}`}
                </span>
              )}
            </div>
            {canEdit && (
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1 bg-muted/50 rounded px-3 py-1.5 w-fit">
                ...
                You can edit the title and abstract while your paper is awaiting
                review
              </div>
            )}
            {!editingTitle ? (
            ...
            <Card className="mb-6 border-orange-400/40 bg-orange-50 dark:bg-orange-950/20">
              <CardContent className="pt-4 pb-4 text-sm space-y-2">
                ...
                <p className="text-muted-foreground">
                  An approval email has been sent to the corresponding author.
                  The paper will proceed to editorial review once approved.
                </p>
          ...
          {paper.status === "ca_rejected" && (
            <Card className="mb-6 border-red-400/40 bg-red-50 dark:bg-red-950/20">
              <CardContent className="pt-4 pb-4 text-sm space-y-2">
                ...
                <p className="text-muted-foreground">
                  The corresponding author has rejected this submission. Please
                  contact them and resubmit if needed.
                </p>
              </CardContent>
            </Card>
          )}
                ...
                {payment ? (
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 bg-white dark:bg-muted/30 rounded-lg p-3 border border-orange-200 dark:border-orange-800/30">
                    <div>
                      ...
                      <p className="font-mono font-semibold text-xs mt-0.5">
                ...
                {(payment?.status === "pending" ||
                  ...
                  <div className="space-y-2">
                    <p className="text-muted-foreground text-xs">
                      After completing the bank transfer, upload your payment
                      receipt below:
                    </p>
                    ...
                    <div className="flex items-start gap-2 text-sm text-green-700 dark:text-green-400">
                      ...
                      <div>
                        <p className="font-medium">
                          Receipt submitted — awaiting publisher approval
                        </p>
                        <p className="text-xs text-muted-foreground mt-0.5">
                          Uploaded on {formatDate(payment.receipt_uploaded_at)}
                        </p>
                      </div>
                    </div>
                  )}
              ...
              <div className="flex items-start gap-0">
                {STAGES.map((stage, i) => {
                  ...
                  const logEntry = status_log.find((l) => {
                    ...
                    if (stage.key === "under_review")
                      return (
                        l.status === "under_review" ||
                        l.status === "assigned_to_sub_editor"
                      );
                    if (stage.key === "decision")
                      return (
                        l.status === "accepted" ||
                        l.status === "rejected" ||
                        l.status === "pending_revision"
                      );
                    if (stage.key === "payment")
                      return (
                        l.status === "awaiting_payment" ||
                        l.status === "payment_review" ||
                        l.status === "ready_for_publication"
                      );
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center w-full">
                        {i > 0 && (
                          <div
                            className={`flex-1 h-0.5 ${completed || active ? "bg-primary" : "bg-muted"}`}
                          />
                        )}
                        <div
                          className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 ${completed ? "bg-primary text-primary-foreground" : active ? "border-2 border-primary text-primary" : "border-2 border-muted text-muted-foreground"}`}
                        >
                          {completed ? (
                      ...
                      <div className="mt-2 text-center px-1">
                        <p
                          className={`text-xs font-medium ${active ? "text-primary" : completed ? "text-foreground" : "text-muted-foreground"}`}
                        >
                          {stage.label}
                        </p>
                        {logEntry && (
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {formatDate(logEntry.changed_at)}
                          </p>
                        )}
                        {stage.key === "published" && publication?.doi && (
                          <a
                            ...
                            className="text-xs text-primary hover:underline flex items-center justify-center gap-0.5 mt-0.5"
                          >
          ...
          <Card className="mb-6">
            <CardContent className="p-4 grid grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium">{formatDate(paper.submitted_at)}</p>
              </div>
              <div>
                <p className="text-muted-foreground text-xs">Last Updated</p>
                <p className="font-medium">{formatDate(paper.updated_at)}</p>
              </div>
              <div>
                <p className="text-muted-foreground text-xs">
          ...
          <Card className="mb-6">
            ...
            <CardContent className="space-y-4 text-sm">
              ...
              {paper.corresponding_authors &&
                paper.corresponding_authors.length > 0 && (
                  <div>
                    ...
                    <>
                      ...
                      {!canEdit && (
                        <p className="text-xs text-muted-foreground mt-1 italic">
                          Title and abstract cannot be edited once the review
                          process has started.
                        </p>
                      )}
                    </>
          ...
          {paper.status === "pending_revision" && (
            <Card className="mb-6 border-amber-500/30 bg-amber-500/5">
              ...
              <CardContent className="space-y-3">
                {ae_decision?.comments && (
                  <div className="rounded-md border border-amber-200 dark:border-amber-800/40 bg-white dark:bg-muted/20 p-3 space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-wrap">
                      {renderPolicyContent(ae_decision.comments)}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      {ae_decision.ae_name} ·{" "}
                      {formatDate(ae_decision.decided_at)}
                    </p>
                  </div>
                )}
                <p className="text-sm text-muted-foreground">
                  Please address the comments above and upload a revised
                  manuscript.
                </p>
                <div>
                  <Button
                ...
                <p className="text-xs text-muted-foreground">
                  Accepted formats: .docx, .pdf, .tex, .latex · max 10MB
                </p>
              </CardContent>
            </Card>
          )}
      ...
      {/* Revision upload confirmation modal */}
      <Dialog open={showRevisionConfirm} onOpenChange={cancelRevisionUpload}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Confirm Revision Upload</DialogTitle>
          </DialogHeader>

          <div className="space-y-3 py-2">
            <p className="text-sm text-muted-foreground">
              You are about to upload this revised manuscript:
            </p>

            {selectedRevisionFile && (
              <div className="rounded-md border bg-muted/40 p-3 text-sm space-y-1">
                ...
                <p>
                  ...
                  {(selectedRevisionFile.size / 1024).toFixed(1)} KB
                </p>
              </div>
            )}
          </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={cancelRevisionUpload}>
              Cancel
            </Button>
            <Button onClick={confirmRevisionUpload} disabled={uploading}>
              {uploading ? (
      ...
      <Dialog open={showReceiptConfirmModal} onOpenChange={cancelReceiptUpload}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            <p className="text-sm text-muted-foreground">
              You are about to upload the following file as your payment
              receipt:
            </p>
            {selectedReceiptFile && (
            ...
            <p className="text-xs text-muted-foreground">
              Once submitted, the publisher will review your receipt and approve
